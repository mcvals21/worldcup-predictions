...
STAGE_LABELS = {
    'group':'المجموعات', 'round32':'دور الـ32', 'round16':'دور الـ16',
...
def participant_page(token):
    ...
    predictions = {x.match_id: x for x in Prediction.query.filter_by(participant_id=p.id).all()}
    champion = ChampionPick.query.filter_by(participant_id=p.id, tournament_id=t.id).first()
    ...
    if request.method == 'POST':
        ...
        if action == 'predict':
            ...
            if is_double and current_double_used(p.id, t.id, match.stage, match.id):
                ...
                return redirect(url_for('participant_page', token=token))
            pred = predictions.get(match.id) or Prediction(participant_id=p.id, match_id=match.id)
            pred.home_score, pred.away_score, pred.is_double = hs, aw, is_double
            ...
            if t.champion_pick_deadline and now_kw() >= t.champion_pick_deadline:
                ...
                if team:
                    pick = champion or ChampionPick(participant_id=p.id, tournament_id=t.id)
                    pick.team_name = team
...
def leaderboard():
    ...
    for p in participants:
        ...
        pts = sum(points_for(pred, Match.query.get(pred.match_id)) for pred in preds)
        exact = sum(1 for pred in preds if Match.query.get(pred.match_id).home_score is not None and pred.home_score == Match.query.get(pred.match_id).home_score and pred.away_score == Match.query.get(pred.match_id).away_score)
        rows.append({'name':p.name,'points':pts,'exact':exact})
...
def match_view(match_id):
    ...
    preds = db.session.query(Prediction, Participant).join(Participant).filter(Prediction.match_id==match_id).all()
    return render_template('match.html', match=match, preds=preds, points_for=points_for)

@app.route('/admin/<code>', methods=['GET','POST'])
def admin(code):
    if code != ADMIN_CODE: abort(404)
    t = tournament()
    if request.method == 'POST':
        action = request.form.get('action')
        if action == 'add_match':
            dt = datetime.strptime(request.form['start_time'], '%Y-%m-%dT%H:%M')
            m = Match(tournament_id=t.id, home_team=request.form['home_team'].strip(), away_team=request.form['away_team'].strip(), start_time=dt, stage=request.form['stage'])
            db.session.add(m); db.session.commit(); flash('تمت إضافة المباراة.')
        elif action == 'result':
            m = Match.query.get_or_404(int(request.form['match_id']))
            m.home_score = int(request.form['home_score']); m.away_score = int(request.form['away_score'])
            db.session.commit(); flash('تم حفظ النتيجة وتحديث النقاط.')
        elif action == 'champion_deadline':
            t.champion_pick_deadline = datetime.strptime(request.form['deadline'], '%Y-%m-%dT%H:%M')
            db.session.commit(); flash('تم حفظ موعد إغلاق توقع البطل.')
        return redirect(url_for('admin', code=code))
    matches = Match.query.filter_by(tournament_id=t.id).order_by(Match.start_time).all()
    participants = Participant.query.order_by(Participant.name).all()
    return render_template('admin.html', t=t, matches=matches, participants=participants, code=code, stage_labels=STAGE_LABELS)
...
def init_db():
    ...
    for name in PARTICIPANT_NAMES:
        db.session.add(Participant(name=name, token=secrets.token_urlsafe(8)))
    db.session.commit()
